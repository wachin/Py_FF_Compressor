...
def show_help(file_name):
    try:
        with open(f'/src/messages/{file_name}', 'r') as file:
            content = file.read()
...
def run_ffmpeg():
    ...
    ffmpeg_process = subprocess.Popen(shlex.split(command), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    ...
    if ffmpeg_process.poll() is None:
        while True:
            ...
            if output == '' and ffmpeg_process.poll() is not None:
                break
            if output:
                ffmpeg_output.insert(tk.END, output.strip() + '\n')
